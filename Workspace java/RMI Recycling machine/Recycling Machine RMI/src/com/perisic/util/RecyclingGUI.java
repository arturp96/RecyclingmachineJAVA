...
import com.perisic.peripherals.RecyclingMachineRMIImplementation;
...
public class RecyclingGUI extends JFrame implements ActionListener, PrinterInterface  {
	...
	CustomerPanel myPanel = new CustomerPanel(this); //creating a new instance of the customer panel class
	SQL test  = new SQL();// creating a new instance of the sql class
	ResultSet rs;// initializing the resultset from the sql class

	
	public void actionPerformed(ActionEvent e) { // this is the action performed which is used by all the buttons
		...
		if(e.getSource().equals(slot1)){
			...
			textArea.setText("Can Inserted!");// if a button called slot1 is pressed then all the action below are run.
			try {
				test.addData("Can");// this line needs to be in a try and catch because it adds data to the sqlite dabase and it needs to catch errors.
			...
			if(myPanel.Weight()<=100) {
				progressBar.setValue((int)myPanel.Weight());// adds the correct weight value towards the progressbar
			}else {
...
//	JButton slot7 = new JButton("SQL Data");
	...
	public RecyclingGUI() {//this is the gui constructor
		super();
...
//		panel.add(slot7);
	...
	public static void main(String [] args ) { //main  method of Recycling gui class
		//everything in this method gets run as soon as the program is run
		
		
		RecyclingGUI myGUI = new RecyclingGUI(); // new instance of the recycling gui class
		myGUI.setVisible(true); // making the recycling gui visible every time the program is run
		try {

			RecyclingMachineRMIImplementation recyclingImpl = new RecyclingMachineRMIImplementation();//creating a new instance of the RMI
			...
			reg.rebind("RecyclingService1718",recyclingInterface);//initilizing the connection with client the clinet gui using rmi
